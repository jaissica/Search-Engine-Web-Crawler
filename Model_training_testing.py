from module.data_processing import QREL
...
class Model:
    ...
    def output_test(self, query, test_set, version):
        with open("./output_files/test_{}.txt".format(version), "a") as f:
            for q_id in query:
    ...
    def output_train(self, query, train_set, version):
        with open("./output_files/train_{}.txt".format(str(version)), "a") as f:
            for q_id in query:
